    ...
    public function searchStudent(Request $request)
        {
            ...
            $studentscount = Student::count();
            $remainders = Remainder::where('user_id',auth()->id())->where('status',1)->whereDate('followup_date',today())->get();
            $students = Student::where('name','like','%'.$search.'%')->orWhere('email','like','%'.$search.'%')->orWhere('contact','like','%'.$search.'%')->orWhere('studentid','like','%'.$search.'%')->orWhere('lastname','like','%'.$search.'%')->orWhere('middlename','like','%'.$search.'%')->orWhere('firstname','like','%'.$search.'%')->simplePaginate(15);
            return view('viewstudent',compact('students','studentscount','remainders'));
        }
